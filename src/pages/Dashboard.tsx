...
// Aggregate session scores across all students into a class-level trend
const classTrend = (() => {
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-10 space-y-10 animate-fade-in">
        ...
        <section className="rounded-2xl p-5"
          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest mb-0.5" style={{ color: '#94a3b8' }}>Class pulse</p>
              <p className="text-sm font-bold" style={{ color: '#0C1825', fontFamily: "'Bricolage Grotesque', system-ui, sans-serif" }}>
                Engagement trend · last 10 sessions
              </p>
            </div>
            <div className="text-right">
              <p className="text-lg font-extrabold" style={{ color: trendColor, fontFamily: "'Bricolage Grotesque', system-ui, sans-serif" }}>
                {trendDelta >= 0 ? '+' : ''}{trendDelta} pts
              </p>
              <p className="text-xs" style={{ color: '#94a3b8' }}>vs. session 1</p>
            </div>
          </div>
          <ResponsiveContainer width="100%" height={110}>
            <AreaChart data={classTrend} margin={{ top: 4, right: 4, left: -28, bottom: 0 }}>
              <defs>
              ...
              <XAxis dataKey="date" tick={{ fontSize: 10, fill: '#cbd5e1' }} tickLine={false} axisLine={false} interval="preserveStartEnd" />
              <Tooltip
                contentStyle={{ background: '#0C1825', border: 'none', borderRadius: 10, padding: '6px 12px' }}
                labelStyle={{ color: 'rgba(255,255,255,0.5)', fontSize: 11 }}
                itemStyle={{ color: '#2dd4bf', fontSize: 12, fontWeight: 700 }}
                formatter={(v: unknown) => [`${v} avg`, '']}
              />
              <Area type="monotone" dataKey="avg" stroke="#0F766E" strokeWidth={2} fill="url(#trendGrad)" dot={false} activeDot={{ r: 4, fill: '#0F766E' }} />
            </AreaChart>
          </ResponsiveContainer>
        </section>
        ...
        {alertStudents.length > 0 && (
          <section>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-xs font-semibold uppercase tracking-widest" style={{ color: '#94a3b8' }}>Priority alerts</h2>
        ...
        <section>
          ...
          >
            ...
            <h2 className="text-xs font-semibold uppercase tracking-widest" style={{ color: '#94a3b8' }}>Topics at a glance</h2>
          ...
          {showTopics && (
            <div className="rounded-2xl overflow-hidden"
              ...
              {topicBreakdown.map((row, i) => {
                ...
                return (
                  <div key={row.topic} className="flex items-center gap-4 px-5 py-3.5"
                    ...
                    <p className="text-sm font-semibold flex-1 truncate" style={{ color: '#0C1825' }}>{row.topic}</p>
                    <div className="hidden sm:flex items-center gap-1.5 flex-shrink-0">
                      ...
                      <span className="text-xs font-bold w-7 text-right" style={{ color: barColor }}>{row.avgScore}</span>
                    </div>
              ...
              {filtered.map((student, i) => (
                <div key={student.id} className="animate-slide-up" style={{ animationDelay: `${i * 30}ms` }}>
                  <StudentCard
                    ...
                    contacted={contacted.has(student.id)}
                  />
                </div>
              ))}
            </div>
